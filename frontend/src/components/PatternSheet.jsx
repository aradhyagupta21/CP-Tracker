...
export default function PatternSheet({ currentUser }) {
  ...
  const renderTopicsView = () => (
    <div className="animate-fadeIn">
      <div className="flex justify-between items-end pb-4 border-b border-slate-800/80 mb-8">
        <div>
          ...
          <h1 className="text-4xl font-extrabold tracking-tight text-slate-100">
            Pattern-Wise <span className="text-slate-100">Mastery</span>
          </h1>
        </div>
        <div className="text-slate-500 text-sm font-medium pb-2">
          <span className="text-slate-100 font-bold text-xl">{leetcodeTopics.length}</span> topics
        </div>
      </div>
        ...
        {leetcodeTopics.map(topic => {
          ...
          return (
            <div 
              ...
              className="bg-[#110e1b] backdrop-blur-md border border-slate-800/80 rounded-2xl p-6 cursor-pointer group hover:bg-[#151122] transition-all duration-300 hover:border-royal/20 hover:-translate-y-1 relative overflow-hidden"
            >
              <div className="flex justify-between items-start mb-6">
                <div className="w-10 h-10 rounded-xl bg-brand-indigo/10 flex items-center justify-center text-brand-indigo group-hover:bg-brand-indigo/10 transition-all">
                  <IconComp className="w-5 h-5" />
                </div>
                <div className="text-xs font-semibold text-slate-500 bg-slate-100 px-2 py-1 rounded-md border border-slate-800/80 group-hover:text-slate-500 transition-colors">
                  {solved}/{topic.totalProblems}
                </div>
              </div>
              <div className="mb-8">
                <h3 className="text-lg font-bold text-slate-100 group-hover:text-brand-indigo mb-1 transition-colors">{topic.title}</h3>
                <p className="text-sm font-medium text-slate-100/70">{topic.patternsCount} patterns</p>
              </div>
              <div className="pt-4 border-t border-slate-800/80">
                <div className="flex justify-between text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-3 group-hover:text-slate-500">
                  <span>Progress</span>
                  <span className={percentage > 0 ? 'text-brand-indigo' : ''}>{percentage}%</span>
                </div>
                <div className="w-full h-1 bg-slate-100 rounded-full overflow-hidden">
                  <div className="h-full bg-brand-indigo/10 rounded-full transition-all duration-1000 ease-out" style={{ width: `${percentage}%` }}></div>
                </div>
              </div>
            </div>
          );
  ...
  const renderPatternsView = () => {
    ...
    return (
      <div className="animate-fadeIn">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between pb-8 mb-8 border-b border-slate-800/80">
          <div>
            <h1 className="text-4xl font-extrabold tracking-tight text-slate-100 mb-2">
              Tactical <span className="text-slate-100">Patterns</span>
            </h1>
            <p className="text-slate-500 text-sm">{currentTopic.patternsCount} patterns — work through each to build complete mastery.</p>
          </div>
          <div className="mt-6 md:mt-0 bg-[#110e1b] border border-slate-800/80 px-6 py-4 rounded-2xl flex items-center gap-6">
            <div className="w-48 h-2 bg-slate-100 rounded-full overflow-hidden relative">
              <div className="absolute inset-y-0 left-0 bg-brand-indigo/10 rounded-full" style={{ width: `${overallPercentage}%` }}></div>
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {currentTopic.subPatterns.map(pattern => {
            ...
            return (
              <div key={pattern.id} className="bg-[#110e1b] border border-slate-800/80 rounded-2xl p-6 flex flex-col hover:border-royal/20 transition-colors group">
                <div className="w-12 h-12 rounded-full bg-slate-50 flex items-center justify-center text-slate-500 mb-6 group-hover:bg-brand-indigo/10 group-hover:text-brand-indigo transition-all">
                  <Play className="w-5 h-5 ml-1" />
                </div>
                <h3 className="text-xl font-bold text-slate-100 mb-1">{pattern.title}</h3>
                ...
                <div className="mb-6">
                  ...
                  <div className="w-full h-1.5 bg-slate-100 rounded-full overflow-hidden">
                    <div className="h-full bg-brand-indigo/10 rounded-full" style={{ width: `${percentage}%` }}></div>
                  </div>
                </div>
                ...
                <div className="mt-auto pt-4">
                  <button 
                    ...
                    className="w-full py-3 bg-slate-50 hover:bg-slate-50 rounded-xl text-sm font-bold text-slate-400 hover:text-brand-indigo border border-transparent hover:border-slate-800/80 transition-all flex items-center justify-center gap-2"
                  >
      ...
      <div className="animate-fadeIn">
        <div className="bg-[#110e1b] border border-slate-800/80 rounded-3xl p-8 mb-8">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center">
            <div>
              <h1 className="text-4xl font-extrabold text-slate-100 mb-2">Pattern Problems</h1>
              <p className="text-slate-500 text-sm">{solved} of {currentPattern.totalProblems} problems solved in this pattern.</p>
            </div>
            <div className="mt-6 md:mt-0 flex items-center gap-4">
              <div className="text-5xl font-black text-slate-100">{percentage}%</div>
              <div>
        ...
        <div className="bg-[#110e1b] border border-slate-800/80 rounded-3xl overflow-hidden">
          ...
          <div className="p-6 border-b border-slate-800/80 flex flex-col md:flex-row items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              <h2 className="text-xl font-bold text-slate-100">Problems</h2>
              <span className="bg-slate-50 px-2 py-1 rounded text-xs font-bold text-brand-indigo">{currentPattern.problems.length} problems</span>
            </div>
            <div className="relative w-full md:w-64">
              ...
              <input 
                ...
                className="w-full bg-slate-100 border border-slate-800/80 rounded-xl pl-9 pr-4 py-2.5 text-sm text-slate-100 focus:outline-none focus:border-brand-indigo/20 focus:ring-1 focus:ring-brand-purple/50 transition-all"
              />
              <div className="absolute right-3 top-1/2 -translate-y-1/2 flex items-center justify-center w-5 h-5 rounded border border-slate-800/80 bg-slate-50 text-[10px] text-slate-500 font-mono">⌘K</div>
            </div>
          </div>
          ...
          <div className="overflow-x-auto">
            {filteredProblems.length === 0 ? (
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="border-b border-slate-800/80 bg-[#110e1b]">
                ...
                <tbody>
                  {filteredProblems.map((prob) => {
                    ...
                    return (
                      <tr key={prob.id} className="border-b border-slate-800/80 hover:bg-slate-50 transition-colors group">
                        <td className="px-6 py-5">
                          <button 
                            onClick={() => toggleProblemStatus(prob.id, currentTopic.id)}
                            className={`flex items-center justify-center w-6 h-6 rounded-full border transition-all ${isSolved ? 'bg-emerald-500/20 border-emerald-500/50 text-emerald-400' : 'border-slate-600 text-transparent hover:border-brand-indigo/20'}`}
                          >
                            {isSolved && <CheckCircle2 className="w-4 h-4" />}
                          </button>
                        </td>
                        <td className="px-6 py-5">
                          <span className="font-bold text-slate-100 group-hover:text-brand-indigo transition-colors">{prob.title}</span>
                        </td>
                        <td className="px-6 py-5 text-center">
                          <a 
                            ...
                            target="_blank" 
                            rel="noreferrer"
                            className="inline-flex items-center justify-center w-8 h-8 rounded-lg bg-slate-50 hover:bg-slate-700 text-slate-500 hover:text-brand-indigo transition-colors"
                          >
                        ...
                        <td className="px-6 py-5 text-right">
                          <button
                            onClick={() => toggleProblemStatus(prob.id, currentTopic.id)}
                            className={`px-5 py-2 rounded-xl text-xs font-bold transition-all shadow-lg hover:shadow-xl ${isSolved ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 hover:bg-emerald-500/20' : 'bg-brand-indigo/10 text-slate-100 hover:bg-[#8b5cf6]'}`}
                          >
