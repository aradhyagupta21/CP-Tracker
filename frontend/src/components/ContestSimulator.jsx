...
export default function ContestSimulator({ currentUser }) {
  ...
  return (
    <div className="space-y-8 animate-fadeIn">
      {/* Header */}
      <div>
        <h1 className="text-4xl font-extrabold tracking-tight bg-gradient-to-r from-brand-cyan via-brand-indigo to-brand-purple bg-clip-text text-transparent flex items-center gap-3">
          <Terminal className="w-8 h-8 text-brand-indigo" />
          Virtual Contest Simulator
        </h1>
        <p className="text-slate-400 mt-1">Generate a custom realistic Codeforces contest and evaluate your performance.</p>
      </div>
      ...
      {!activeContest && !evaluation && (
        <div className="glass-panel p-8 rounded-2xl border border-slate-700 glow-indigo max-w-xl mx-auto mt-12 text-center space-y-6">
          <Target className="w-16 h-16 text-brand-cyan mx-auto animate-bounce-slow" />
          <h2 className="text-2xl font-bold text-slate-100">Configure Your Contest</h2>
          <p className="text-slate-400 text-sm">Select your division to generate 4 problems with an appropriate difficulty curve (A, B, C, D).</p>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              {['Div 1', 'Div 2', 'Div 3', 'Div 4'].map(div => (
                <button
                  ...
                  className={`py-3 rounded-xl font-bold text-sm transition border ${selectedDivision === div ? 'bg-brand-indigo/20 border-brand-indigo text-brand-indigo shadow-[0_0_15px_rgba(99,102,241,0.2)]' : 'bg-dark-900 border-slate-700 text-slate-400 hover:border-slate-500 hover:text-slate-200'}`}
                >
                  {div}
                </button>
              ))}
          ...
          <button 
            ...
            disabled={isLoading}
            className="w-full flex items-center justify-center gap-2 bg-gradient-to-r from-brand-indigo to-brand-purple hover:opacity-95 text-slate-100 px-6 py-3 rounded-xl font-bold transition disabled:opacity-50"
          >
            {isLoading ? (
      ...
      {!activeContest && !evaluation && currentUser && (
        <div className="glass-panel p-8 rounded-2xl border border-slate-700 max-w-xl mx-auto mt-6">
          ...
          <div className="grid grid-cols-2 gap-4 mb-6">
            ...
            <div className="bg-dark-900/50 p-4 rounded-xl border border-slate-800">
              ...
              <p className="text-3xl font-extrabold text-brand-cyan mt-1">
          ...
          {records.length > 0 && (
            <div className="space-y-3 max-h-60 overflow-y-auto pr-2 custom-scrollbar">
              {records.slice(0, 5).map(r => (
                <div key={r._id} className="flex justify-between items-center bg-dark-950 p-3 rounded-lg border border-slate-800/80">
                  <div>
                    <span className="text-sm font-bold text-slate-200">{r.division}</span>
                    <span className="text-xs text-slate-500 ml-2">
                      {new Date(r.createdAt).toLocaleDateString()}
                    </span>
                  </div>
                  <div className="flex items-center gap-4 text-sm font-medium">
                    <span className="text-slate-300">{r.solvedCount} / 4 Solved</span>
      ...
      {activeContest && !evaluation && (
        <div className="space-y-6">
          <div className="glass-panel p-6 rounded-2xl border border-brand-indigo/30 bg-brand-indigo/5 flex flex-col md:flex-row justify-between items-center gap-4">
            <div>
              <h2 className="text-2xl font-bold text-slate-100">Virtual Contest: Target {activeContest.targetRating}</h2>
              <p className="text-sm text-brand-cyan font-medium mt-1">Generated {activeContest.problems.length} custom problems.</p>
            </div>
            <div className="flex items-center gap-4 bg-dark-900 border border-slate-700 px-6 py-3 rounded-xl">
          ...
          <div className="grid grid-cols-1 gap-4">
            {activeContest.problems.map((prob, index) => (
              <div key={prob.letter} className="glass-panel p-6 rounded-2xl border border-slate-700 flex flex-col lg:flex-row gap-6 justify-between items-start lg:items-center">
                <div className="space-y-2 flex-1 min-w-0">
                  <div className="flex items-center gap-3">
                    <span className="text-xl font-black text-brand-purple w-6">{prob.letter}</span>
                    <a href={prob.url} target="_blank" rel="noreferrer" className="text-lg font-bold text-slate-200 hover:text-brand-cyan transition line-clamp-1">
                      {prob.name}
                    </a>
                    <span className="text-xs px-2 py-0.5 rounded bg-slate-800 text-slate-400 border border-slate-700 font-medium">*{prob.rating}</span>
                  </div>
                ...
                <div className="flex flex-wrap items-center gap-4 w-full lg:w-auto bg-dark-900/50 p-4 rounded-xl border border-slate-800/80">
                  <label className="flex items-center gap-2 cursor-pointer shrink-0">
                    <input 
                      type="checkbox" 
                      checked={results[index].solved}
                      onChange={(e) => handleResultChange(index, 'solved', e.target.checked)}
                      className="w-5 h-5 rounded border-slate-700 text-brand-cyan accent-brand-cyan cursor-pointer"
                    />
                    <span className={`text-sm font-bold ${results[index].solved ? 'text-emerald-400' : 'text-slate-400'}`}>Solved</span>
                  </label>
                    ...
                    <>
                      <div className="flex items-center gap-2">
                        ...
                        <input 
                          ...
                          className="w-16 bg-dark-950 border border-slate-700 rounded px-2 py-1 text-sm text-slate-200 text-center outline-none"
                          placeholder="e.g. 12"
                        />
                      </div>
                      <div className="flex items-center gap-2">
                  ...
                  {!results[index].solved && (
                    <button 
                      ...
                      className="text-xs font-bold text-brand-indigo hover:text-brand-cyan transition px-3 py-1.5 border border-brand-indigo/30 hover:border-brand-cyan/50 rounded-lg"
                    >
                      Mark Solved Now
                    </button>
                  )}
          ...
          <div className="flex justify-end gap-4 mt-6">
            <button 
              onClick={handleReset}
              className="px-6 py-3 rounded-xl border border-slate-700 text-slate-400 font-bold hover:text-slate-200 hover:bg-slate-800 transition"
            >
              Abort Contest
            </button>
            <button 
      ...
      {evaluation && (
        <div className="space-y-6">
          <div className="glass-panel p-8 rounded-2xl border border-brand-purple/30 bg-gradient-to-br from-dark-900 to-dark-950 text-center relative overflow-hidden">
            ...
            <Trophy className="w-20 h-20 text-brand-purple mx-auto mb-6 drop-shadow-[0_0_15px_rgba(168,85,247,0.4)]" />
            
            <h2 className="text-3xl font-extrabold text-slate-100 mb-2">Contest Evaluation Complete</h2>
            <p className="text-slate-400 font-medium max-w-lg mx-auto">Based on your solution speeds, penalties, and target difficulty curve, here is your estimated performance:</p>
            
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-4xl mx-auto mt-10">
              <div className="glass-panel p-6 rounded-2xl border border-slate-700/60 bg-dark-950/40 glow-indigo relative z-10">
                <p className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Estimated Rank</p>
              ...
              <div className="glass-panel p-6 rounded-2xl border border-slate-700/60 bg-dark-950/40 glow-indigo relative z-10">
                <p className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Rating Change</p>
              ...
              <div className="glass-panel p-6 rounded-2xl border border-slate-700/60 bg-dark-950/40 glow-indigo relative z-10">
                ...
                <div className="text-4xl font-black text-brand-cyan">{evaluation.totalScore}</div>
            ...
            <button 
              onClick={handleReset}
              className="mt-10 px-8 py-3 rounded-xl border border-slate-600 bg-slate-800/50 hover:bg-slate-700/50 text-slate-200 font-bold transition flex items-center gap-2 mx-auto relative z-10"
            >
